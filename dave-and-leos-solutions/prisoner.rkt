...
(define (extract-entry a-play seq)
  (if (equal? (caar seq) a-play)
    (car seq)
    (extract-entry a-play (cdr seq))))
...
(define (EYE-FOR-TWO-EYES my-history other-history)
  (if (or (empty-history? my-history) (empty-history? (rest-of-plays my-history)))
    "c"
    (if (and (string=? "d" (most-recent-play other-history))
...
;    (define (iter cnt other-history)
;      (if (string=? "c" (most-recent-play other-history))

(define (make-eye-for-n-eyes depth)
  (lambda (my-history other-history)
    (define (count-instances-of test hist depth)
      (cond ((empty-history? hist) 0)
            ((= depth 0) 0)
            ((string=? (most-recent-play hist) test)
             (+ (count-instances-of test (rest-of-plays hist) (- depth 1)) 1))
            (else (count-instances-of (rest-of-plays hist (- depth 1))))))
    (if (= depth (count-instances-of "d" other-history depth)) "d" "c")))

(define (make-rotating-strategy strat1 strat2 freq1 freq2)
  (lambda (my-history other-history)
    (if (< (remainder (length my-history) (+ freq1 freq2)) freq1)
      (strat1 my-history other-history)
      (strat2 my-history other-history))))

(define (make-higher-order-spastic strategies)
  (lambda (my-history other-history)
    (define (nth-strategy n)
      (car (drop strategies n)))
    ((nth-strategy (remainder (length my-history) (length strategies))) my-history other-history)))
...
(define PANTSY (make-rotating-strategy NASTY PATSY 1 1))

(display "PANTSY:\n")
(play-loop PANTSY NASTY)
(play-loop PANTSY PATSY)

(display "\nNASTY-EGAL:\n")
(define NASTY-EGAL (make-rotating-strategy NASTY EGALITARIAN 1 1))
(play-loop NASTY-EGAL NASTY)
(play-loop NASTY-EGAL PATSY)
(play-loop NASTY-EGAL EYE-FOR-EYE)
(play-loop NASTY-EGAL SPASTIC)
(play-loop NASTY-EGAL EGALITARIAN)


(display "\nSPAS1:\n")
(define SPAS1 (make-higher-order-spastic (list PATSY NASTY)))
(play-loop SPAS1 NASTY)
